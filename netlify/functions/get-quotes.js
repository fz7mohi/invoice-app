...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    console.log('Query parameters:', { clientId, status, page, itemsPerPage, lastDocId });
    ...
    const totalQuery = query(collection(db, 'quotes'));
    const totalSnapshot = await getDocs(totalQuery);
    const totalQuotes = totalSnapshot.size;

    console.log(`Retrieved ${quotes.length} quotes out of ${totalQuotes} total`);
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        currentPage: page,
        itemsPerPage,
        hasMore: quotes.length === itemsPerPage
      })
    };
  } catch (error) {
